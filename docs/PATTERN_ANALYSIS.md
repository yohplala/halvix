...
**Symbol Replacement Detection**: Providers sometimes reuse ticker symbols for different tokens (e.g., old "MOVE" token replaced by Movement Labs "MOVE"). The analyzer detects these replacements and uses only post-replacement data. See `detect_symbol_replacement` in `src/data/price_filters.py`.
