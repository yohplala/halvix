...
class SymbolMappingCache:
    ...
    def _prefetch_coingecko_prices(
        ...
        try:
            ...
            self._coingecko_prices_cache = {
                c.id: c.current_price_btc or 0.0
                for c in coins
            }
        except Exception:
    ...
    def validate_mapping(
        ...
        if not is_valid and not error_message:
            error_message = f"Price difference {price_diff:.2f}% exceeds tolerance {self.tolerance_percent}%"
    ...
    def validate_batch(
        ...
        if show_progress:
            print(f"Prefetching CoinGecko prices...")
